...
# Forward Chaining untuk menentukan penyakit dari gejala dan trisemester
...
def create_pengguna():
    try:
        ...
        # Validasi nama
        required_fields = ['nama', 'usiaKandungan', 'pekerjaan', 'alamat']
...
def get_trimester():
    try:
        ...
        query = "SELECT * FROM trimester ORDER BY id"
        cursor.execute(query)
...
def get_gejala():
    try:
        ...
        query = "SELECT * FROM gejala ORDER BY id"
        cursor.execute(query)
...
def create_diagnosa():
    try:
        ...
        gejala_terpilih = [int(g) for g in data['gejala'] if g]  #Ngefilter yang kosong amjay
        
        if not gejala_terpilih:
            return jsonify({'error': 'No symptoms selected'}), 400
        
        # Diaplikasi kan euy ke forward chaining
        ...
        # Kalo ini diagnosis symthomnye
        for gejala_id in gejala_terpilih:
            query_gejala = """
            INSERT INTO diagnosis_gejala (id_diagnosa, id_gejala) 
            VALUES (%s, %s)
            """
            cursor.execute(query_gejala, (diagnosa_id, gejala_id))
